...
bool IndirectDrawingApplication::initialize()
{
    ...
    std::string shaderPath = std::string(ROOT_PATH) + "Experiments/IndirectDrawing/shaders/indirect";
    ...
    std::vector<std::string> modelFiles = {
        std::string(ASSETS_PATH) + "Models/Untracked/attack_droid.obj",
        std::string(ASSETS_PATH) + "Models/monkey.3ds"};
    for (const auto& modelFile : modelFiles)
    {
        ...
        numMeshes += static_cast<unsigned int>(model.getMeshes().size());
        ++numModels;
...
void IndirectDrawingApplication::createCommandBuffer(const std::vector<fw::Model>& models)
{
    ...
    for (const auto& model : models)
    {
        for (const auto& mesh : model.getMeshes())
        {
            drawCmd[cmdIndex].count = static_cast<GLuint>(mesh.indices.size());
