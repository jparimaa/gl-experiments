...
bool ImageLoadStoreApplication::initialize()
{
    ...
    std::string path = std::string(ROOT_PATH) + "Experiments/ImageLoadStore/shaders/";
    if (!loadShader(path + "loadstore", &loadStoreShader) || !loadShader(path + "display", &displayShader))
    {
        return false;
    }
