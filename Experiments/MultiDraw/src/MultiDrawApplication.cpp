...
bool MultiDrawApplication::initialize()
{
    ...
    std::string shaderPath = std::string(ROOT_PATH) + "Experiments/MultiDraw/shaders/simple";
    ...
    fw::Model model;
    std::string modelFile = std::string(ASSETS_PATH) + "Models/sponza.obj";
...
void MultiDrawApplication::render()
{
    ...
    if (renderMultiBuffer)
    {
        ...
        glMultiDrawElements(GL_TRIANGLES, multi.counts.data(), GL_UNSIGNED_INT, (GLvoid**)multi.indexLocations.data(), static_cast<GLsizei>(multi.counts.size()));
        ++drawCalls;
    }
    else
    {
...
{
    distinct.numBuffers = static_cast<GLsizei>(model.getMeshes().size());
    ...
    for (GLsizei i = 0; i < distinct.numBuffers; ++i)
    {
        ...
        distinct.numIndices[i] = static_cast<GLsizei>(mesh.indices.size());
...
void MultiDrawApplication::createMultiBuffer(const fw::Model& model)
{
    ...
    {
        GLuint indexOffset = static_cast<GLsizei>(tempVertices.size());
        ...
        multi.indexLocations.push_back(static_cast<GLsizei>(tempIndices.size()) * sizeof(GLuint));
        for (const auto& i : mesh.indices)
        {
            tempIndices.push_back(i + indexOffset);
        }
        multi.counts.push_back(static_cast<GLsizei>(mesh.indices.size()));
    }
